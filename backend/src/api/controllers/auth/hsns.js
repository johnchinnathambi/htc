...
const schemaName = HSN
const schemaNameString = 'HSN'

export const getHSNs = async (req, res) => {
  try {
...
export const postHSN = async (req, res) => {
  try {
...
export const putHSN = async (req, res) => {
  try {
    const { id } = req.params
    const { hSNCodeSerialNo, hSNCode, description } = req.body

    const object = await schemaName.findById(id)
    if (!object)
      return res.status(400).json({ error: `${schemaNameString} not found` })

    object.hSNCodeSerialNo = hSNCodeSerialNo
