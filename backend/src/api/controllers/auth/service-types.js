...
const schemaName = ServiceType
const schemaNameString = 'ServiceType'
...
export const postServiceType = async (req, res) => {
  try {
...
export const putServiceType = async (req, res) => {
  try {
    const { id } = req.params
    const { serviceSerialNo, serviceName, particular, fees, status} = req.body

    const object = await schemaName.findById(id)
    if (!object)
      return res.status(400).json({ error: `${schemaNameString} not found` })

    object.serviceSerialNo = serviceSerialNo
