...
export const putUnit = async (req, res) => {
  try {
    const { id } = req.params

    const { unitSerialNo, unitName } = req.body

    const object = await schemaName.findById(id)
    if (!object)
      return res.status(400).json({ error: `${schemaNameString} not found` })

    object.unitSerialNo = unitSerialNo
    object.unitName = unitName
...
export const deleteUnit = async (req, res) => {
  try {
    ...
    //     role.cityUnit.filter((item) => item.toString() !== id).length
    //     await role.save()
    //   })
    // }
